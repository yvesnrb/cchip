...
void cls (Machine *machine, byte nibbles[4]);
 ...
 */
void ret (Machine *machine, byte nibbles[4]);

/* 1nnn - Jump to location nnn.
   The interpreter sets the program counter to nnn.
 */
void jp_addr (Machine *machine, byte nibbles[4]);

/* 2nnn - Call subrotine at nnn.
   The interpreter increments the stack pointer, then puts the current
   PC on the top of the stack. The PC is then set to nnn.
 */
void call_addr (Machine *machine, byte nibbles[4]);

/* 3xkk - Skip next instruction if vx = kk.
   The interpreter compares register Vx to kk, and if they are equal,
   increments the program counter by 2.
 */
void se_vx_byte (Machine *machine, byte nibbles[4]);

/* 4xkk - Skip next instruction if vx != kk.
   The interpreter compares register Vx to kk, and if they are not
   equal, increments the program counter by 2.
 */
void sne_vx_byte (Machine *machine, byte nibbles[4]);

/* 5xy0 - Skip next instruction if vx = vy.
   The interpreter compares register Vx to register Vy, and if they
   are equal, increments the program counter by 2.
 */
void se_vx_vy (Machine *machine, byte nibbles[4]);

/* 6xkk - Set vx = kk.
   The interpreter puts the value kk into register Vx.
 */
void ld_vx_byte (Machine *machine, byte nibbles[4]);

/* 7xkk - Set vx = vx + kk.
   Adds the value kk to the value of register Vx, then stores the
   result in Vx.
 */
void add_vx_byte (Machine *machine, byte nibbles[4]);

/* 8xy0 - Set vx = vy.
   Stores the value of register Vy in register Vx.
 */
void ld_vx_vy (Machine *machine, byte nibbles[4]);
 ...
 */
void or_vx_vy (Machine *machine, byte nibbles[4]);
...
/* 8xy2 - Set Vx = Vx AND Vy.
   ...
   also 1. Otherwise, it is 0.
 */
void and_vx_vy (Machine *machine, byte nibbles[4]);
...
/* 8xy3 - Set Vx = Vx XOR Vy.
   ...
   0.
 */
void xor_vx_vy (Machine *machine, byte nibbles[4]);
...
/* 8xy4 - Set Vx = Vx + Vy, set VF = carry.
   ...
   Vx.
 */
void add_vx_vy (Machine *machine, byte nibbles[4]);

/* 8xy5 - Set Vx = Vx - Vy, set VF = NOT borrow.
   If Vx > Vy, then VF is set to 1, otherwise 0. Then Vy is subtracted
   from Vx, and the results stored in Vx.
 */
void sub_vx_vy (Machine *machine, byte nibbles[4]);

/* 8xy6 - Set Vx = Vx SHR 1.
   If the least-significant bit of Vx is 1, then VF is set to 1,
   otherwise 0. Then Vx is divided by 2.
 */
void shr_vx_vy (Machine *machine, byte nibbles[4]);

/* 8xy7 - Set Vx = Vy - Vx, set VF = NOT borrow.
   If Vy > Vx, then VF is set to 1, otherwise 0. Then Vx is subtracted
   from Vy, and the results stored in Vx.
 */
void subn_vx_vy (Machine *machine, byte nibbles[4]);

/* 8xyE - Set Vx = Vx SHL 1.
   If the most-significant bit of Vx is 1, then VF is set to 1,
   otherwise to 0. Then Vx is multiplied by 2.
 */
void shl_vx_vy (Machine *machine, byte nibbles[4]);

/* 9xy0 - Skip next instruction if Vx != Vy.
   The values of Vx and Vy are compared, and if they are not equal,
   the program counter is increased by 2.
 */
void sne_vx_vy (Machine *machine, byte nibbles[4]);

/* Annn - Set I = nnn.
   The value of register I is set to nnn.
 */
void ld_i_addr (Machine *machine, byte nibbles[4]);

/* Bnnn - Jump to location nnn + V0.
   The program counter is set to nnn plus the value of V0.
 */
void jp_v0_addr (Machine *machine, byte nibbles[4]);
 ...
 */
void rnd_vx_byte (Machine *machine, byte nibbles[4]);
...
/* Dxyn - Display n-byte sprite starting at memory location I at (Vx, Vy), set VF = collision.
   ...
   information on XOR.
 */
void drw_vx_vy_nibble (Machine *machine, byte nibbles[4]);

/* Ex9E - Skip next instruction if key with the value of Vx is pressed.
   Checks the keyboard, and if the key corresponding to the value of Vx
   is currently in the down position, PC is increased by 2.
 */
void skp_vx (Machine *machine, byte nibbles[4]);

/* ExA1 - Skip next instruction if key with the value of Vx is not pressed.
   Checks the keyboard, and if the key corresponding to the value of
   Vx is currently in the up position, PC is increased by 2.
 */
void sknp_vx (Machine *machine, byte nibbles[4]);

/* Fx07 - Set Vx = delay timer value.
   The value of DT is placed into Vx.
 */
void ld_vx_dt (Machine *machine, byte nibbles[4]);

/* Fx0A - Wait for a key press, store the value of the key in Vx.
   All execution stops until a key is pressed, then the value of that
   key is stored in Vx.
 */
void ld_vx_k (Machine *machine, byte nibbles[4]);

/* Fx15 - Set delay timer = Vx.
   DT is set equal to the value of Vx.
 */
void ld_dt_vx (Machine *machine, byte nibbles[4]);

/* Fx18 - Set sound timer = Vx.
   ST is set equal to the value of Vx.
 */
void ld_st_vx (Machine *machine, byte nibbles[4]);

/* Fx1E - Set I = I + Vx.
   The values of I and Vx are added, and the results are stored in I.
 */
void add_i_vx (Machine *machine, byte nibbles[4]);

/* Fx29 - Set I = location of sprite for digit Vx.
   The value of I is set to the location for the hexadecimal sprite
   corresponding to the value of Vx.
*/
void ld_f_vx (Machine *machine, byte nibbles[4]);
 ...
 */
void ld_b_vx (Machine *machine, byte nibbles[4]);

/* Fx55 - Store registers V0 through Vx in memory starting at location I.
   The interpreter copies the values of registers V0 through Vx into
   memory, starting at the address in I.
 */
void ld_i_vx (Machine *machine, byte nibbles[4]);
